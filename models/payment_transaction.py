...
class TransactionOsb(models.Model):
    ...
    osb_trans_status = fields.Char('Transaction status')
    osb_card_brand = fields.Char('Means of payment')
    osb_card_number = fields.Char('Card number')
    osb_expiration_date = fields.Char('Expiration date')
    osb_auth_result = fields.Char('Authorization result')
    osb_raw_data = fields.Text(string='Transaction log', readonly=True)
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        values = self.provider_id.osb_form_generate_values(processing_values)
        ...
        values['osb_signature'] = self.provider_id._osb_generate_sign(self, values)
        values['api_url'] = self.provider_id.osb_get_form_action_url()
        return values

    def _osb_get_tx_from_notification_data(self, notification_data):
    ...
    def _process_notification_data(self, notification_data):
        ...
        values = {
            ...
            'osb_raw_data': '{}'.format(notification_data),
            'osb_html_3ds': html_3ds,
            'osb_trans_status': notification_data.get('vads_trans_status'),
            'osb_card_brand': notification_data.get('vads_card_brand'),
            'osb_card_number': notification_data.get('vads_card_number'),
