...
class ProviderOsb(models.Model):
    ...
    def osb_get_doc_field_value():
        ...
        for lang, doc_uri in docs_uri.items():
            html = '<a href="%s%s">%s</a> '%(doc_uri,'odoo17/sitemap.html', constants.OSB_DOCUMENTATION.get(lang))
            doc_field_html += html

        return doc_field_html

    def _get_entry_modes(self):
        modes = constants.OSB_PAYMENT_DATA_ENTRY_MODE
    ...
    osb_redirect_success_timeout = fields.Char(string='Redirection timeout on success', help='Time in seconds (0-300) before the buyer is automatically redirected to your website after a successful payment.')
    osb_redirect_success_message = fields.Char(string='Redirection message on success', help='Message displayed on the payment page prior to redirection after a successful payment.', default='Redirection to shop in a few seconds...')
    osb_redirect_error_timeout = fields.Char(string='Redirection timeout on failure', help='Time in seconds (0-300) before the buyer is automatically redirected to your website after a declined payment.')
    osb_redirect_error_message = fields.Char(string='Redirection message on failure', help='Message displayed on the payment page prior to redirection after a declined payment.', default='Redirection to shop in a few seconds...')
    osb_return_mode = fields.Selection(string='Return mode', help='Method that will be used for transmitting the payment result from the payment page to your shop.', selection=[('GET', 'GET'), ('POST', 'POST')])
    osb_multi_warning = fields.Boolean(compute='_osb_compute_multi_warning')

    osb_multi_count = fields.Char(string='Count', help='Installments number')
    osb_multi_period = fields.Char(string='Period', help='Delay (in days) between installments.')
    osb_multi_first = fields.Char(string='1st installment', help='Amount of first installment, in percentage of total amount. If empty, all installments will have the same amount.')

    osb_test_password = fields.Char(string='Test password', help=constants.OSB_REST_API_KEYS_DESC)
    osb_prod_password = fields.Char(string='Production password', help=constants.OSB_REST_API_KEYS_DESC)
    osb_public_test_key = fields.Char(string='Public test key', help=constants.OSB_REST_API_KEYS_DESC)
    ...
    def _osb_payment_config(self, amount):
    ...
    def _osb_get_rest_password(self):
        if self.state == 'test':
            return str(self.osb_test_password)

        return str(self.osb_prod_password)

    def _osb_get_rest_public_key(self):
        if self.state == 'test':
            return str(self.osb_public_test_key)

        return str(self.osb_public_production_key)

    def _osb_get_rest_sha256_key(self):
        if self.state == 'test':
            return str(self.osb_sha256_test_key)

        return str(self.osb_sha256_prod_key)

    def _osb_get_javascript_server_url(self):
        return constants.OSB_PARAMS.get('STATIC_URL') + "js/krypton-client/V4.0/stable/kr-payment-form.min.js"

    def _osb_get_embedded_stylesheet_url(self):
        return constants.OSB_PARAMS.get('STATIC_URL') + "js/krypton-client/V4.0/ext/" + self.osb_embedded_theme + "-reset.css"

    def _osb_get_embedded_stylesheet_script_url(self):
    ...
    def _osb_get_embedded_payment_means(self):
        cards = []
        for value in self.osb_payment_cards:
    ...
    def _osb_get_inline_form_values(
        ...
        values = {
            ...
            "entry_mode": self.osb_payment_data_entry_mode,
            "pop_in": self.osb_embedded_pop_in,
            "compact": self.osb_embedded_compact_mode
        }
    ...
    def _should_build_inline_form(self, is_validation=True):
        ...
        if (self.code == 'osb'):
            if (str(self.osb_payment_data_entry_mode) != 'redirect'):
                return True
