...
paymentForm.include({
        ...
        $(document).ready(function() {
            ...
            var observer = new MutationObserver( function(mutations) {
                mutations.forEach((mutation) => {
                    const checkedRadio = document.querySelectorAll("[data-payment-method-code='osb']")[0];
    ...
    _osbGetInlineValues() {
        ...
        const osbInlineForm = inlineForm.querySelector('[name="o_osb_element_container"]');

        return JSON.parse(
            osbInlineForm.dataset['osbInlineFormValues']
        );
    },

    async _prepareInlineForm(providerId, providerCode, paymentOptionId, paymentMethodCode, flow) {
        if ((typeof (KR) == "undefined") || (paymentMethodCode != "osb")) {
            this._super(...arguments);

            return;
        }

        const inlineValues = this._osbGetInlineValues();
        ...
        if (deliveryCarrier !== null) {
            localStorage.removeItem('osbFormToken');
            localStorage.removeItem('osbFormTokenData');
        }

        // If there is already a stored token, check if payment data has changed.
        var formToken = localStorage.getItem('osbFormToken');
        var tokenData = localStorage.getItem('osbFormTokenData');
        if ((formToken !== null) && (tokenData == JSON.stringify(inlineValues))) {
            ...
            this._osbDisplayEmbeddedForm(formToken, inlineValues);
        } else {
            await fetch('/payment/osb/createFormToken', {
                method: "POST",
                headers: {
              ...
              .then(data => {
                ...
                if (data["formToken"] === false) {
                    ...
                    localStorage.setItem('osbFormToken', formToken);
                    localStorage.setItem('osbFormTokenData', JSON.stringify(inlineValues));
                }
    ...
    _osbDisplayEmbeddedForm(formToken, inlineValues) {
        ...
        if (inlineValues["pop_in"] === "1") {
            ...
            document.getElementById("osb-embedded-wrapper").appendChild(embeddedDiv);
        } else {
    ...
    async _processDirectFlow(providerCode, paymentOptionId, paymentMethodCode, processingValues) {
        ...
        var formToken = localStorage.getItem('osbFormToken');
        var inlineValues = this._osbGetInlineValues();

        if (localStorage.getItem('osbFormTokenData') == JSON.stringify(inlineValues)) {
    ...
    async _initiatePaymentFlow(providerCode, paymentOptionId, paymentMethodCode, flow) {
        if (providerCode == "osb" && ! can_process_payment) {
            this._osbSubmitPayment();
